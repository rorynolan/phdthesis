...
The parameter $l$ of a boxcar is such that the length of the boxcar is equal to $2l + 1$. This ensures that the length of the boxcar is always odd) which means it can always be centered upon a point). Hence the allowable lengths of a boxcar are $3,5,7,9,$ etc. 
...
(ref:exp-smooth-func-caption) The function $\exp(-\frac{|t|}{\tau})$ visualized with $\tau = 3$ and $\tau = 9$. For $\tau = 3$, points at distance $|t| = 10$ have approximately zero weight, whereas for $\tau = 9$, these points have significant weight.
