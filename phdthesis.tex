...
positive correlation between the intensities of these two colour
...
instrument can go fast enough (with a laser alternation time far below
the residence time of the diffusing entities), this problem is
negligible.
...
400 downloads per month which amounts to 4,000 since it was first
released.
...
each with 100,000 diffusing fluorescent particles which are uniformly
distributed at the beginning. In the first image series (\texttt{img1}),
these have brightness \(\epsilon = 4\) and in the second (\texttt{img2})
they have brightness \(\epsilon = 7\). See figure
...
like Quian and Elson's moment analysis assume photon count data, which
is necessarily in \(\mathbb{N}_0\)). When current methods were failing
to properly detrend low-intensity images, I began to wonder was it
