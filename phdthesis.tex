...
people. \texttt{ijtiff} gets 400 downloads per month which amounts to
4,000 since it was first released.
...
same direction at the same time, there'll be a positive correlation; if
they deviate from their means in the opposite direction at the same
time, there'll be a negative correlation. Correlation is all about
...
using the \emph{automatic} detrending method. Assuming thae population
was all monomers before ligand addition, the \(\approx 1.6\)-fold
increase upon addition suggests that dimerization had occurred (see
figure \ref{fig:FKBPfoldchanges}), however, we expected the increase to
be \(\approx 2\)-fold upon dimerization. There could be many reasons for
this discrepancy.
...
\tightlist
\item
  FKBP is natively expressed in Cos7 cells, so depending on the level of
  native expression (not labelled) and the level of expression of our
  transfected FKBP (labelled), it could be that many cases of what is
  actually a dimer is behaving like a monomer because only one of the
  pair is labelled.
\item
