...
The two electronic states (S~0~ and S~1~) of a molecule are defined as the highest occupied molecular orbital (S~0~, HOMO) and the lowest unoccupied molecular orbital (S~1~, LUMO). Each electronic state has various associated _vibrational_ states between which _non-radiative_ transitions can occur. An electron can be _excited_ from the ground state S~0~ into the higher energy excited state S~1~ by absorption of a photon of the appropriate energy. The electron will remain in the excited state (possibly undergoing non-radiative traansitions through vibrational states) for a period of nanoseconds before _relaxing_ to the lower energy ground state, losing energy by means of emmitting a photon of that energy. This is shown by means of a _Jablonski_ diagram (figure \@ref(fig:Jablonski)).

(ref:Jablonski-caption) Jablonski diagram of the process of fluorescence. Absorption causes excitation, relaxation causes the emmision of light.
...
```{definition}
The quantum efficiency of a detector is the fraction of fluorescent signal that is reported by the detector.
```
...
(ref:widefield-caption) Light travels from the uniformally illuminated sample through a lens (or a few lenses) to the camera. Not the simplicity of this setup relative to that of a confocal microscope.
...
Scientific CMOS (sCMOS) cameras consist of a CMOS circuit bonded to a CCD imaging substrate. These relatively new cameras offer very low noise, rapid frame rates, large dynamic range, high resolution and wide field of view, all cheaper than a regular CCD [@sCMOS]. sCMOS cameras appear to better than anything that has come before.
...
It is essential that the pixel dwell time is the same for each pixel, particularly for FFS. This is something that must be carefully verified on each instrument.
...
The N&B technique is fraught with technical difficulties. Principal of these is the problem of photobleaching. Most of my PhD focussed on corrections for photobleaching. This is discussed in chapter \@ref(photobleaching-correction). 
...
1. In FCS, it's very common for people to mistake correlation for cross-correlation. This is unfortunate, but knowing about this common mistake is essential for navigating the field in a sensible manner. It seems that when people in the FCS field correlate the signals from two separate channels, they use the term _cross_-correlation, even though they're only using correlation. I think the idea of working _across_ two or more channels (and ideas such as _cross_-talk) leads to this confustion.
...
* measure the stoichiometry of the interaction of HIV-1 with its receptor and co-recptor over time in the pre-fusion process [@HIVstoichiometry]

FCS has been used to:

* reveal structural and functional properties of promyelocytic leukima nuclear bodies [@Hoischen]
* demonstrate that HIV-1 evades antibody-dependent phagocytosis [@Gach]
* determine the size of nanodomains [@Fenz]
* perform chromatograaphic measurements [@Kisley]
